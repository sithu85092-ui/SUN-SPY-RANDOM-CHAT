...
(() => {
    ...
    $("#forgotPasswordBtn")
      ?.addEventListener(
        ...
        () => {
          ...
          toast(
            "Demo: password reset instructions would be emailed."
          );
  ...
  function verifyOtp() {
    ...
    toast(
      "Email verified. Welcome to SUN SPY!"
    );
  ...
  function resendOtp() {
    ...
    $("#demoOtpText")
      && (
        $("#demoOtpText")
          .textContent =
          `Demo verification code: ${state.otp}`
      );
...
async function startRandomMatch() {
  ...
  try {
    ...
    /* Keep the matching UI visible so the user can see what failed. */
    const title = $("#matchingTitle");
    const status = $("#matchingStatus");
    const message = error?.message || "Unable to start matching.";

    if (title) title.textContent = "Matching paused";
    if (status) {
      status.textContent = message.includes("signaling") || message.includes("Signaling")
        ? "Signaling server is not connected yet."
        : message.includes("getUserMedia") || message.includes("Camera")
